...
function generate(): void {
    ...
    router.post(
        RecipeController.share.url(props.recipeId),
        {},
        {
            preserveScroll: true,
            preserveState: true,
...
function revoke(): void {
    if (
        !window.confirm(
            'Deel-link intrekken? De bestaande link werkt dan niet meer.',
        )
    ) {
        return;
    }
    ...
    router.delete(RecipeController.unshare.url(props.recipeId), {
        preserveScroll: true,
        preserveState: true,
        only: ['recipe'],
        onFinish: () => {
...
<template>
    <Dialog v-model:open="open">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                <DialogTitle>Deel dit recept</DialogTitle>
                <DialogDescription>
            ...
            <div v-else class="flex flex-col gap-3">
                <p class="text-sm text-ink-soft">
                    Er is nog geen actieve deel-link. Genereer er een om dit
                    recept te delen via WhatsApp, mail, of waar dan ook.
                </p>
                <Button
                    type="button"
                    :disabled="processing"
